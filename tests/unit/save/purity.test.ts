/**
 * WO-013 — domain purity (layering constraint).
 *
 * src/domain/save must stay pure TypeScript: no Phaser, React, Zod, or Node.js
 * built-ins; only the dialogue and quest content domains may be imported.
 */
...
describe('WO-013 save domain purity', () => {
  const domainDir = resolve(__dirname, '../../../src/domain/save');

  it('src/domain/save stays framework- and infra-free', () => {
  ...
  it("domain/save only imports sibling modules, '../dialogue', and '../quest'", () => {
    for (const f of listTs(domainDir)) {
      ...
      for (const line of text.split('\n')) {
        ...
        expect(
          ['../dialogue', '../quest'],
          `${f}: disallowed sibling import "${target}"`
        ).toContain(target);
      }
    }
  });
