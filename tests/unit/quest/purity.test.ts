/**
 * FS-QUEST-001 — domain purity (layering constraint).
 *
 * src/domain/quest must stay pure TypeScript: no Phaser, React, Zod, or Node.js
 * built-ins; only the events and content domains may be imported.
 */
...
describe('WO-012 quest domain purity', () => {
  const domainDir = resolve(__dirname, '../../../src/domain/quest');

  it('src/domain/quest stays framework- and infra-free', () => {
  ...
  it('domain/quest only imports sibling modules and the events/content domains', () => {
    for (const f of listTs(domainDir)) {
      ...
      for (const line of text.split('\n')) {
        ...
        if (target === '../events' || target === '../content') continue;
